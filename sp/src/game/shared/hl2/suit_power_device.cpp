...
using BaseDeviceClass = C_SuitPowerDevice;

#ifdef VKZ_ADVANCED_SPRINT
using SprintDeviceClass = C_SprintDevice;
#endif
...
BEGIN_RECV_TABLE_NOBASE(BaseDeviceClass, DT_SuitPowerDevice)
	RecvPropDevice(),
END_RECV_TABLE()
#endif
...
using BaseDeviceClass = CSuitPowerDevice;

#ifdef VKZ_ADVANCED_SPRINT
using SprintDeviceClass = CSprintDevice;
#endif
...
BEGIN_SEND_TABLE_NOBASE(BaseDeviceClass, DT_SuitPowerDevice)
	SendPropDevice(),
END_SEND_TABLE()
#endif
...
BEGIN_DATADESC_NO_BASE(BaseDeviceClass)
...
BEGIN_DATADESC(SprintDeviceClass)
...
#ifdef VKZ_ADVANCED_SPRINT
const SprintDeviceClass SprintDeviceClass::Default(bits_SUIT_DEVICE_SPRINT, DEFAULT_SPRINT_DRAIN_RATE, DEFAULT_SPRINT_SPEED);
#endif
